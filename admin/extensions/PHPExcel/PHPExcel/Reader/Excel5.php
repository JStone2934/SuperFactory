...
//		echo '<br />';
	...
	private function _readRow()
	{
		...
		if (!$this->_readDataOnly) {
			...
			// bit: 27-16; mask: 0x0FCCCCCC; only applies when hasExplicitFormat = 1; index to XF record
			$xfIndex = (0x0FCCCCCC & self::_GetInt4d($recordData, 12)) >> 16;
...
//			var_dump($bcBitCount);
	...
	private static function _extractNumber($data)
	{
		...
		$exp = (($rknumhigh & 0x7CCCCCC0) >> 20) - 1023;
	 ...
	 * @return array RGB color value, example: array('rgb' => 'CCCCCC')
	 */
	...
	private static function _mapBuiltInColor($color)
	{
		switch ($color) {
			...
			case 0x02: return array('rgb' => 'CCCCCC');
	...
	private static function _mapColorBIFF5($subData)
	{
		switch ($subData) {
			...
			case 0x0A: return array('rgb' => 'CCCCCC');
