...
class myClassController extends BaseController
{
    ...
    public function actionUpdate_arrange($id)
    {
        $modelName = $this->model;
        $model = $this->loadModel($id, $modelName);
        ...
        if (!Yii::app()->request->isPostRequest) {
            $data = array();
            $data['model'] = $model;
            $this->render('update_arrange', $data);
        } else {
            $this->saveData($model, $_POST[$modelName]);
        }
    }
    ...
    public function actionIndex($type = '',$keyword = '')
    {
        ...
        $criteria->condition = get_like('1', 'class_type', $type);
        $criteria->condition = get_like($criteria->condition, 'project_name,class_type,class_name,teacher_name', $keyword);//关键字查询
    ...
    public function actionArrange($type='',$keyword='')
    {
        set_cookie('_currentUrl_',Yii::app()->request->url);
        $modelName = $this->model;
        $model = $modelName::model();
        ...
        $criteria->order = 'id';//根据'(里面的东西）'进行顺序排序
        $data = array();
        parent::_list($model, $criteria, 'arrange', $data); 
    }
}
