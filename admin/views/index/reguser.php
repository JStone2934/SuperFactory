...
<head>
    <meta charset="utf-8">
    <title>广东教育学会中小学生阅读研究评审系统</title>
...
<input type="hidden" name="user_login" value="<?php echo $user_login;?>">
    ...
    <?php  $form = $this->beginWidget('CActiveForm', get_form_login()); ?>
        <div class="item"><h1>广东教育学会中小学生阅读研究评审系统</h1></div>
        <?php 
         if($user_login=='1'){
        ?>
<input type="hidden" name="Teacher_TPWD" value="<?php echo $TPWD;?>">
<input type="hidden" name="TUNAME" id="TUNAME" value="<?php echo $TUNAME;?>">
        ...
        <div class="item">
            ...
            <?php echo $form->error($model, 'F_ROLENAME', $htmlOptions = array()); ?>
               <a class="btn" href="<?php echo $this->createUrl('regUser');?>"><i class="fa fa-plus"></i>注册</a>
                   <a class="btn" href="<?php echo $this->createUrl('reSetPass');?>"><i class="fa fa-plus"></i>忘记密码</a>
        </div><!--item end-->
...
function login() {
      ...
      var s2='<?php echo $this->createUrl("index/checkUser");?>';
       $.ajax({
        type: 'get',
        url: s2,
        data: {USERNAME: s1,PASSWORD:$("#Teacher_TPWD").val(),
        ROLE:$("#Teacher_F_ROLENAME").val(),
        ...
        //   alert("密码 或 用户身份 错误");
          if (s1>0){
             var s22='<?php echo $this->createUrl("index/index&TCOD=");?>'+s1;
