...
public function title($vt,$vt1='') {
 ...
 function extArray($flag,$ext,$img='')//flag判断图片(0)还是文件+图片(1)
 {
     if($flag)
     {
         $path=array(
         ...
         if(array_key_exists($ext,$path)) return BasePath::model()->getImagePath().$path[$ext].'"style="max-height:40px;max-width:50px;">';
         else if(BaseLib::extJudge("jpg,gif,jpeg,png",$ext)) return $img.'" style="max-height:60px; max-width:40px;">';
         else return BasePath::model()->getImagePath().'image/fail.png'.'" style="max-height:30px; max-width:20px;text-align:center;">'.'文件格式错误';
     }
     else {
         if(BaseLib::extJudge("jpg,gif,jpeg,png", $ext)) return $img.'" style="max-height:120px; max-width:80px;">';
         else return BasePath::model()->getImagePath().'image/fail.png'.'" style="max-height:30px; max-width:20px;text-align:center;">'.'文件格式错误';
     }
 }
 //输入形如pdf,docx的字符串和【后缀名】，返回对后缀名的判断
 function extJudge($str,$file)
 {
     $ext = explode(",",$str);
     return in_array($file,$ext);
 }
...
//传入图片地址，id名（update用）
//2022-3-27参考达良：
//$flag ：1是能传文件+图片 0 是只能传图片,$file传入图片地址，id名（update用）

function show_pic($flag,$flie='',$id=''){
  //put_msg(185);
    $html='';
    $attach=explode(".",$flie);
    $ext = end($attach);//获取后缀名
    //put_msg($flie);
    //put_msg(strlen(BasePath::model()->getWwwPath())+1);
    if(strlen($flie)>=strlen(BasePath::model()->getWwwPath())+1){
      //put_msg(191);
        $html=empty($id)?'<div style="text-align:center">':
            '<div style="float: left; margin-right:10px" id="upload_pic_'.$id.'">';
        if((BaseLib::extJudge("pdf,docx,doc,zip,rar,xlsx,xls", $ext) &&$flag) || BaseLib::extJudge("jpg,png,gif,jpeg", $ext))
            $html.= '<a href="'.$flie.'" target="_blank" title="点击查看">';
        else
            $html.= '<a href=" " target="_blank" title="格式错误">';
        //这里防止用户下载错误格式的文件，增加程序鲁棒性

        $html.= BaseLib::extJudge("jpg,gif,jpeg,png", $flie) ?'< img src="'.$flie.'" style="max-height:80px; max-width:70px;">':'';
        if($flag)
        {
          //put_msg(203);
            $html.= '< img src="'. BaseLib::extArray(1, $ext);
        }
        else
        {
          //put_msg(BaseLib::extArray(0, $ext,$flie));
            $html.= '<img src="'. BaseLib::extArray(0, $ext,$flie);
