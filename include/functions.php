...
function get_check_code($pcheck_code){
    ...
    if ($pcheck_code == 'shenhe') {
        $f_check= 371;
    } else if ($pcheck_code == 'baocun' || $pcheck_code == 'quxiao') {
        $f_check= 721;
    } else if ($pcheck_code == 'tongguo') {
        $f_check= 372;
    } else if ($pcheck_code == 'butongguo') {
        $f_check= 373;
    } else if ($pcheck_code == 'tuihui') {
        $f_check= 1538;
    }
    return $f_check;
}
...
function hsYii_selectByData($form,$m,$atts,$onchange='',$noneshow=''){
     return BaseLib::model()->selectByData($form,$m,$atts,$this->gData(),'bankName',$onchange,$noneshow);
}
   
function hsYii_searchBy($title,$filedname){
     return BaseLib::model()->searchBy($title,$filedname,$this->gData(),'bankName');
}
